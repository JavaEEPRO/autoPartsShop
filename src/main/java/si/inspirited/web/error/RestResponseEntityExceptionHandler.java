...
@Override
protected ResponseEntity<Object> handleMethodArgumentNotValid(final MethodArgumentNotValidException exc, final HttpHeaders headers, final HttpStatus status, final WebRequest request) {
final BindingResult res = exc.getBindingResult();
        logger.error("400 Status Code", exc);
final GenericResponse bodyOfResponse = new GenericResponse(res.getAllErrors(), "Invalid" + res.getObjectName());
