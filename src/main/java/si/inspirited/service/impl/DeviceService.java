...
import si.inspirited.persistence.dao.DeviceMetadataRepository;
...
public class DeviceService {
    ...
    private String getIpLocation(String ip) throws IOException, GeoIp2Exception { //, GeoIp2Exception {
        if("0:0:0:0:0:0:0:1".equalsIgnoreCase(ip)) {ip="0:0:0:0:0:ffff:c18e:db69";}/*2a02:7aa0:1201::84a7:1e1c";}*/        //for local tests))
    ...
    private void unknownDeviceNotification(String deviceDetails, String location, String ip, String email, Locale locale) {
        ...
        String text = messages
                ...
                " " + ip;
          //final Message notification = constructEmailMessage("j2eengineer@gmail.com", subject, text); //new SimpleMailMessage();
        //notification.setText(text);
        //notification.setFrom(from);

        //mailSender.send(notification);        //sends email with device data
        /*try {
            Transport.send(notification);//msg);
        }catch (MessagingException me) {me.printStackTrace();}
        */
         emailService.constructAndSend(email, subject, text);
    }
}
